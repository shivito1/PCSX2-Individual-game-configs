...
if __name__ == "__main__":
    ...
    # print("Number of arguments: " + str(num_argv))
    if len(sys.argv) - 1 < 1:
        Mainisgo(Tk())
    else:
        for i in range(len(sys.argv)):
            print("arg#:%s " % (i) + sys.argv[i])
            if "--fullscreen" in sys.argv[i]:
                fullscreen = sys.argv[i]
            elif sys.argv[i].endswith(('.gz', '.iso', ".7z")):
                rom_file = sys.argv[i]
            elif "--cfgpath" in sys.argv[i]:
                game_config = sys.argv[i]
            ...
            try:  # used try so that if user pressed other than the given key error will not be shown
                if time.time() <= timeout:
                    if keyboard.is_pressed('l'):  # if key 'q' is pressed
                        # all of theys sys.argv need to be identifyed to prevent crashes
                        print(pcsx2location + " " + fullscreen + ' "' + rom_file + '" ')
                        subprocess.Popen(
                            pcsx2location + " " + fullscreen + ' "' + rom_file + '" ')  # play the fucking game!
                        Mainisgo(Tk())
                        break  # finishing the loop
                    elif keyboard.is_pressed('z'):
                        subprocess.Popen(
                            pcsx2location + " " + sys.argv[1] + ' ' + sys.argv[2] + ' "' + sys.argv[3] + '" ')
                        break
                    else:
                        pass
                else:
